//
//  ARCameraViewModelYOLO.swift
//  VocabApp
//
//  Enhanced with YOLO11n Object Detection
...
final class ARCameraViewModelYOLO: NSObject, ObservableObject {
    @Published var englishLabel: String = ""
    @Published var vietnameseLabel: String = ""
    @Published var ipaLabel: String = ""
    @Published var confidence: Double = 0.0
    @Published var isRunning: Bool = false
    @Published var detections: [DetectedObject] = []   // all objects for drawing boxes

    let session = ARSession()
    
    private let yoloService = YOLOObjectDetectionService()
    private var lastInferenceTime: TimeInterval = 0
    private let inferenceInterval: TimeInterval = 0.20  // ~5 fps
    ...
    override init() {
        ...
        print("🟢 [YOLO VIEWMODEL] Initialized with YOLO11n service")
    }

    func start() {
        guard ARWorldTrackingConfiguration.isSupported else { 
            print("❌ [YOLO VIEWMODEL] AR not supported on this device")
            return 
        }
        ...
        print("✅ [YOLO VIEWMODEL] Started - Using YOLO11n for detection")
        print("ℹ️  [YOLO VIEWMODEL] Inference interval: \(self.inferenceInterval)s (~\(Int(1.0/self.inferenceInterval)) FPS)")
        print("ℹ️  [YOLO VIEWMODEL] Lexicon entries: \(LexiconManager.entries.count) words")
    }

    func stop() {
        session.pause()
        DispatchQueue.main.async { self.isRunning = false }
        print("🛑 [YOLO VIEWMODEL] Stopped")
    }

    private func handleYOLO(_ results: [DetectedObject]) {
        guard !results.isEmpty else { return }
        
        print("🔍 [YOLO DETECTION] Found \(results.count) objects")
        
        // Lọc các object liên quan đến từ vựng
        let vocabularyObjects = yoloService.filterVocabularyObjects(results)
        guard !vocabularyObjects.isEmpty else { 
            print("⚠️  [YOLO DETECTION] No vocabulary objects found")
            return 
        }
        
        print("✅ [YOLO DETECTION] \(vocabularyObjects.count) vocabulary objects found")
        
        // Publish all detections for overlay
        DispatchQueue.main.async { self.detections = vocabularyObjects }

        // Lấy detection tốt nhất
        guard let bestDetection = vocabularyObjects.first else { return }
        
        let raw = bestDetection.className.lowercased().trimmingCharacters(in: .whitespacesAndNewlines)
        let pickedConf = Double(bestDetection.confidence)
        
        print("🎯 [YOLO DETECTION] Best: '\(raw)' (confidence: \(String(format: "%.1f", pickedConf * 100))%)")
        
        // Lookup trong từ điển
        guard let chosen = Self.lookup(rawLabel: raw) else {
            // Nếu không có trong từ điển, hiển thị tên gốc
...
extension ARCameraViewModelYOLO: ARSessionDelegate {
    func session(_ session: ARSession, didUpdate frame: ARFrame) {
        ...
        // Sử dụng YOLO detection
        yoloService.detectObjects(in: buffer, orientation: .right) { result in
            switch result {
            case .success(let objects):
                self.handleYOLO(objects)
            case .failure(let error):
                print("YOLO detection error: \(error.localizedDescription)")
            }
        }
    }
}
